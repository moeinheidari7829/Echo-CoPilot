#!/usr/bin/env python3
"""
Test agent stability with SINGLE LLM (no self-contrast).
This tests the agent with tools but WITHOUT the self-contrast mechanism.
"""
...
def run_single_iteration(
    ...
    """Run agent on all examples once with SINGLE LLM (no self-contrast)."""
    print(f"\n{'='*60}")
    ...
    for idx, example in enumerate(examples):
        ...
        try:
            # USE SINGLE LLM (no self-contrast)
...
def run_stability_test(
    ...
    """Run stability test with SINGLE LLM (no self-contrast)."""
    ...
    output_dir = Path(output_base_dir) / f"agent_single_llm_{measurement_tool}_{timestamp}"
    ...
    print(f"{'='*60}")
    print(f"Agent Stability Test (SINGLE LLM - NO SELF-CONTRAST)")
    print(f"{'='*60}")
    print(f"Dataset: {dataset_path}")
    print(f"Examples: {num_examples} (starting at {start_idx})")
    print(f"Iterations: {num_iterations}")
    print(f"Measurement tool: {measurement_tool}")
    print(f"Mode: Single LLM with tools (NO self-contrast)")
    ...
    # Initialize agent once with custom log directory
    print(f"\nInitializing agent (single LLM mode)...")
    ...
    final_report = {
        "metadata": {
            "test_type": "agent_single_llm",
            "measurement_tool": measurement_tool,
            "mode": "single_llm_with_tools_no_self_contrast",
    ...
    print(f"\nAgent Stability Test Results (SINGLE LLM):")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test agent stability with single LLM (no self-contrast)")
