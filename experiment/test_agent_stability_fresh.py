#!/usr/bin/env python3
"""
Test agent stability with FRESH agent initialization for each iteration.
Tests if agent reuse causes instability.
"""
...
def run_single_iteration(
    ...
    """Run agent on all examples once with FRESH agent initialization."""
    print(f"\n{'='*60}")
    print(f"Iteration {iteration_num}")
    print(f"{'='*60}")
    
    # Initialize FRESH agent for this iteration
    print(f"Initializing fresh agent for iteration {iteration_num}...")
    agent = IntelligentAgent(log_dir=trajectories_dir)
    ...
    for idx, example in enumerate(examples):
        ...
        try:
            response = agent.process_query_with_self_contrast(
                query=query,
                video_path=video_path
            )

            extracted_answer = extract_answer_from_text(response.response_text)
...
def run_stability_test(
    ...
    """Run stability test with FRESH agent for each iteration."""
    ...
    output_dir = Path(output_base_dir) / f"agent_fresh_{measurement_tool}_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)

    # Create logs and trajectories subdirectories
    ...
    # Extract subset (random or sequential)
    import random
    ...
    print(f"Agent will be reinitialized for EACH iteration\n")
    ...
    final_report = {
        "metadata": {
            "test_type": "agent_stability_fresh_init",
            "measurement_tool": measurement_tool,
            "agent_initialization": "fresh_per_iteration",
    ...
    print(f"\nStability Test Results (FRESH INITIALIZATION):")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test agent stability with fresh initialization per iteration")
