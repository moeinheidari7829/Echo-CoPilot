#!/usr/bin/env python3
"""
Test LLM with tools (no agent framework, no self-contrast).
This gives the LLM direct access to tools via function calling.
"""
...
LLM_WITH_TOOLS_SYSTEM_PROMPT = """
You are an expert echocardiography assistant with access to analysis tools.

You have access to these tools:
- echo_segmentation: Segment cardiac structures (LV, LA, RA, RV)
- echo_measurement_prediction: Predict measurements (EF, volumes, etc.)
- echo_disease_prediction: Predict disease presence and severity
- echo_view_classification: Classify echocardiographic view

For each question:
1. Decide which tool(s) to use based on the question
2. Call the appropriate tools with the video path
...
```json
{
  ...
  "reasoning": "Brief explanation based on tool outputs"
}
```
"""
...
def run_single_iteration(
    ...
    """Run LLM with tools on all examples once with a ReAct loop."""
    print(f"\n{'='*60}")
    ...
    for idx, example in enumerate(examples):
        ...
        try:
            messages = [
                SystemMessage(content=LLM_WITH_TOOLS_SYSTEM_PROMPT),
                HumanMessage(content=f"Question: {query}\n\nVideo path: {video_path}\n\nPlease use the available tools to analyze this echocardiogram and answer the question.")
            ]
            ...
            for step in range(max_steps):
                ...
                if not response.tool_calls:
                    ...
                    if extracted_answer:
                        break
                    # If no answer found but no tool calls, it might just be a text response without JSON
                    # We'll continue one more step if possible or just stop
                    if step == max_steps - 1:
                        break
                    continue
            ...
            result = {
                ...
                "response_text": response_text[:1000] if response_text else "No final response",
                "success": extracted_answer is not None,
                "steps_taken": len(messages) - 2 # Exclude initial system and human messages
            }
...
def run_llm_with_tools_test(
    dataset_path: str,
    num_iterations: int = 10,
    num_examples: int = 50,
    start_idx: int = 0,
    measurement_tool: str = "echoprime",
    output_base_dir: str = "experiment/stability_tests"
) -> None:
    """Run LLM with tools stability test."""

    # Set measurement tool
    os.environ["MEASUREMENT_TOOL"] = measurement_tool

    # Create output directory
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir = Path(output_base_dir) / f"llm_with_tools_{measurement_tool}_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"{'='*60}")
    print(f"LLM with Tools Stability Test")
    print(f"{'='*60}")
    print(f"Dataset: {dataset_path}")
    print(f"Examples: {num_examples} (starting at {start_idx})")
    print(f"Iterations: {num_iterations}")
    print(f"Model: {Config.OPENAI_MODEL}")
    print(f"Measurement tool: {measurement_tool}")
    print(f"Mode: LLM with tools (no agent framework)")
    ...
    # Extract subset (same random sampling as other tests)
    import random
    ...
    for idx in selected_indices:
        ...
        if video_rel_path.startswith("mimic-iv-echo/"):
            video_rel_path = video_rel_path.replace("mimic-iv-echo/", "")
        video_path = Path("mimic-iv-echo-ext-mimicechoqa-a-benchmark-dataset-for-echocardiogram-based-visual-question-answering-1.0.0/MIMICEchoQA") / video_rel_path
    ...
    print(f"✓ LLM ready with {len(tools)} tools bound")
    ...
    final_report = {
        "metadata": {
            "test_type": "llm_with_tools",
            "model": Config.OPENAI_MODEL,
            "temperature": Config.OPENAI_TEMPERATURE,
            "measurement_tool": measurement_tool,
            "mode": "llm_with_tools_no_agent_framework",
    ...
    print(f"\nLLM with Tools Stability Results:")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test LLM with tools (no agent framework)")
    ...
    parser.add_argument("--measurement-tool", choices=["echoprime", "echonet", "both"], default="echoprime")
    parser.add_argument("--output-dir", default="experiment/stability_tests")

    args = parser.parse_args()

    run_llm_with_tools_test(
