...
"""
This script processes a folder of DICOM files for TAPSE inference.
It outputs annotated images and saves metadata (e.g., TAPSE measurements) to a CSV file.
"""

parser = ArgumentParser()
parser.add_argument("--folders", type=str, required=True, help="Path to folder containing DICOM files")
parser.add_argument("--output_path_folders", type=str, help="Path to output folder for results")
...
for DICOM_FILE in DICOM_FILES:
    try:
        ...
        if PhotometricInterpretation == 'MONOCHROME2':
            ...
            print(f"Unsupported Photometric Interpretation in {DICOM_FILE}")
            continue
        ...
        PhysicalDeltaX_doppler = abs(doppler_region.get((0x0018, 0x602C)).value)
        ...
        with torch.no_grad():
            ...
            # Ensure TAPSE points are valid
            if (point_x1 < point_x2) and (point_y1 < point_y2):
                print(f"Invalid TAPSE point order in {DICOM_FILE}")
                continue

            calculated_TAPSE = np.sqrt(
                (abs((point_x1 - point_x2)) * PhysicalDeltaX_doppler) ** 2 +
                (abs((point_y1 - point_y2)) * PhysicalDeltaY_doppler) ** 2
            ).round(2)
            ...
            results.append({
                ...
                "PhysicalDeltaX": PhysicalDeltaX_doppler,
...
# SAMPLE SCRIPT
#python inference_TAPSE_folders.py  --folders ./SAMPLE_DICOM/TAPSE_FOLDERS --output_path_folders ./OUTPUT/TAPSE
